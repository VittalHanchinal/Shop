...
import { addSareeCustomer } from '../services/api';

export default function SareeFormScreen({ navigation }) {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            const response = await addSareeCustomer(formData);
            if (response.success) {
    ...
    return (
        <View style={styles.container}>
            <View style={styles.header}>
                ...
                <Text style={styles.title}>Add Saree Customer</Text>
                <View style={{ width: 60 }} />
            </View>
                ...
                <View style={styles.form}>
                    ...
                    <View style={styles.inputGroup}>
                        ...
                        <View style={styles.typeSelector}>
                            {['Simple', 'Designer', 'Premium'].map((type) => (
                                <TouchableOpacity
                                    key={type}
                                    style={[
                                        styles.typeButton,
                                        formData.designType === type && styles.activeTypeButton,
                                    ]}
                                    onPress={() => updateField('designType', type)}
                                >
                                    <Text
                                        style={[
                                            styles.typeButtonText,
                                            formData.designType === type && styles.activeTypeButtonText,
                                        ]}
                                    >
                                        {type}
                                    </Text>
